...
export function uniqueId(): string {
	const now = Date.now();
	if (now > latestTime) {
		// 如果当前时间大于最近一次生成的时间，使用当前时间生成
		return v1({msecs: latestTime = now});
	} else {
		// 如果当前时间小于最近一次生成的时间，使用最近一次生成的时间+1生成
		return v1({msecs: ++latestTime});
	}
}

/**
 * 将时间戳转换为日期时间字符串
 * @example parseDatetime(1620000000000) // "2021-05-03 00:00:00"
 * @param timestamp 时间戳
 * @returns 日期时间字符串
 */
export function parseDatetime(timestamp: string | number): string {
	if (!timestamp) {
		return "";
	}
 ...
 * @param value 车牌号码
 */
export function checkCarNumber(value: string): boolean {
